...
module.exports = {
    ...
    handleCalendarDisplayName: function (date, isHoliday, isVication) {
        ...
        if (self.judgeIsHoliday(date)) {
            return config.holiday[self.dateFormat('yyyy-MM-dd', new Date(date))];
        }
        if (isVication) {
        ...
        if (self.isTomorrow(dt)) {
            ...
            let index = Math.round(self.calculateDaysNum(new Date(config.startDate), date));
            let weekIndex = Math.round(index/7) + 1;
            if (index % 7 === 1 && weekIndex <= 20) {
